...
export default function Home() {
    ...
    function calc_market_price(
        ...
        let price = p.toNumber() / 10000000000 / 1000;
        return price;
    }
    ...
    async function getBondingCurveData(): Promise<BondingCurveData> {
        ...
        const provider = new AnchorProvider(
            new Connection(
                "https://devnet.helius-rpc.com/?api-key=29acd0dc-e336-4909-873a-0ed1010a9de2" // devnet
        ...
        console.log("virtualSolReserves: ", accInfo.reserveSol.toString());
        console.log("realTokenReserves: ", accInfo.relReserveToken.toString());
        console.log("realSolReserves: ", accInfo.relReserveSol.toString());
        console.log(
            "当前价格:",
            calc_market_price(accInfo.reserveSol, accInfo.reserveToken)
                .toFixed(10)
                .toString()
        );
        console.log("===================================================");
    ...
    const handleCalculate = async () => {
        ...
        // if (tokenMint.length < 43) {
        //     console.log("invalid tokenMint, length");
        //     return;
        // }
        ...
        try {
            ...
            if (isBuyOperation) {
                if (calcByOption == "calcBySOL") {
                    ...
                    setPayInAmount(
                        (dy.toNumber() / 1000000).toFixed(6).toString()
                    );
                    ...
                    setPayInAmount(
                        (dx.toNumber() / 1000000000).toFixed(6).toString()
                    );
                    setMarketPrice(mprice.toFixed(10).toString());
                ...
                // 计算手续费 1%
                let fee = dx.div(new BN(100));
                setTradeFeeAmount(fee.toString());
                // 计算最新成交价
                let mprice = calc_market_price(
                    bondingCurveData.virtualSolReserves.sub(dx),
                    bondingCurveData.virtualTokenReserves.add(dy)
                );
